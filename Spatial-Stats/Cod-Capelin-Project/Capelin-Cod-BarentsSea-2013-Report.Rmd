...
# Introduction
Northeast Arctic cod (_Gadus morhua_) and Barents Sea capelin (_Mallotus villosus_) are important Norwegian commercial fisheries stocks in the Barents Sea. Cod is a generalist predator, but has a preference for capelin and is the main predator on post-larval capelin. Cod also feeds on immature cod.
...
1. Which factors influence these patterns?
...
All three species co-occur at many sites, but the relative abundance of each species varies at sites \@ref(fig:overlapabun).

The highest abundance and overlap appears in the central region of the survey area.
...
Mature cod are correlated with immature cod and capelin, along with bottom depth, bottom temperature, and pelagic temperature \@ref(fig:corr). Bottom temperature and pelagic temperature are highly correlated, therefore, bottom temperature was not included in the model.
...
```{r lm, echo=FALSE, fig.show="hold", out.width="50%", fig.cap="\\label{fig:figs}Predicted abundance of log(mature cod+1) to A) immature cod and B) capelin in Barents Sea from ordinary kriging."}
#Response: Cod.mat
...
The abundance of immature cod and capelin increases with increasing abundance of mature cod (as log(mature cod + 1)) \@ref(fig:lm). The variances are large, so this model needs to be explored further. The spatial correlation should be incorporated into the model to improve the fit.

Looking at the kriged predicted abundance for mature cod, immature cod, and capelin, mature and immature cod have similar patterns of abundance and spatial distribution \@ref(fig:krigplot). The highest predicted abundance of mature and immature cod largely overlap, with a region of higher abundance to the northeast. It appears that capelin may have higher abundance between 2 regions of high abundance of mature and immature cod.
